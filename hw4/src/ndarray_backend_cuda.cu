...
__device__ size_t gid_to_idx(size_t gid, CudaVec shape, CudaVec strides, size_t offset) {
  size_t idx = offset;
...
__global__ void EwiseSetitemKernel(const scalar_t* a, scalar_t* out, size_t size, CudaVec shape,
  ...
  size_t gid = blockIdx.x * blockDim.x + threadIdx.x;
  if (gid < size)
    out[gid_to_idx(gid, shape, strides, offset)] = a[gid];
}
...
__global__ void ScalarSetitemKernel(const scalar_t val, scalar_t* out, size_t size, CudaVec shape,
  ...
  size_t gid = blockIdx.x * blockDim.x + threadIdx.x;
  if (gid < size)
    out[gid_to_idx(gid, shape, strides, offset)] = val;
}
...
void ScalarMul(const CudaArray& a, scalar_t val, CudaArray* out) {
  CudaDims dim = CudaOneDim(out->size);
  ScalarMulKernel<<<dim.grid, dim.block>>>(a.ptr, val, out->ptr, a.size);
}
...
void EwiseDiv(const CudaArray& a, const CudaArray& b, CudaArray* out) {
  CudaDims dim = CudaOneDim(out->size);
  EwiseDivKernel<<<dim.grid, dim.block>>>(a.ptr, b.ptr, out->ptr, a.size);
}
...
void ScalarDiv(const CudaArray& a, scalar_t val, CudaArray* out) {
  CudaDims dim = CudaOneDim(out->size);
  ScalarDivKernel<<<dim.grid, dim.block>>>(a.ptr, val, out->ptr, a.size);
}
...
void ScalarPower(const CudaArray& a, scalar_t val, CudaArray* out) {
  CudaDims dim = CudaOneDim(out->size);
  ScalarPowerKernel<<<dim.grid, dim.block>>>(a.ptr, val, out->ptr, a.size);
}
...
__global__ void ReduceMaxKernel(const scalar_t* a, scalar_t* out, size_t reduce_size, size_t out_size) {
  size_t gid = blockIdx.x * blockDim.x + threadIdx.x;
  if (gid < out_size) {
    scalar_t reduce_max = a[gid * reduce_size];
    for (int j = 1; j < reduce_size; ++j) {
      if (reduce_max < a[gid * reduce_size + j])
        reduce_max = a[gid * reduce_size + j];
    }
    out[gid] = reduce_max;
  }
}
...
__global__ void ReduceSumKernel(const scalar_t* a, scalar_t* out, size_t reduce_size, size_t out_size) {
  size_t gid = blockIdx.x * blockDim.x + threadIdx.x;
  if (gid < out_size) {
    scalar_t reduce_sum = a[gid * reduce_size];
    for (int j = 1; j < reduce_size; ++j) {
      reduce_sum += a[gid * reduce_size + j];
    }
    out[gid] = reduce_sum;
  }
}
